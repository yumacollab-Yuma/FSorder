...
export default function OverviewPage({ products, daily }: { products: Product[]; daily: DailyEntry[] }) {
  ...
  const activeEntries = mode === 'single'
    ? daily.filter(d => d.date === singleDate)
    : daily.filter(d => d.date >= rangeStart && d.date <= rangeEnd)
  ...
  function buildTrend(entries: DailyEntry[]) {
    ...
    for (const d of trendDates) byDate[d] = { total: 0, organic: 0, paid: 0, refund: 0 }
    for (const e of entries) {
      if (byDate[e.date]) {
      ...
      <div className="bg-[#13151f] border border-[#2a2d45] rounded-xl p-4 mb-6">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="ml-auto text-xs text-[#444870]">
            {mode === 'single' ? singleDate || '—' : `${trendDates.length} 天`}
          </div>
        </div>
      </div>
        ...
        <div className="bg-[#13151f] border border-[#2a2d45] rounded-xl p-5 mb-6">
          ...
          <TrendChart
            ...
            series={[
              ...
              { label: '退货',   color: '#e85d75', values: trend.refund, dash: [4, 3] },
            ]}
          />
        </div>
      )}
        ...
        <div className="bg-[#13151f] border border-[#2a2d45] rounded-xl overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#1c1f2e] border-t border-[#2a2d45]">
                {['#','商品名称','总订单','自然流','广告流','退货','退货率','占比','份额条'].map(h => (
            ...
            <tbody>
              {productTotals.length === 0 ? (
                <tr><td colSpan={9} className="px-4 py-8 text-center text-xs text-[#444870]">该时间段内无数据</td></tr>
                ...
                return (
                  <tr key={p.pid} className="border-t border-[#2a2d45] hover:bg-[rgba(255,255,255,0.02)]">
                    ...
                    <td className="px-3 py-2.5 tabular-nums text-[#3ecf8e]">
                      {p.organic.toLocaleString()} <span className="text-[11px] text-[#444870]">{orgPct}%</span>
                    </td>
                    <td className="px-3 py-2.5 tabular-nums text-[#f5a623]">
                      {p.paid.toLocaleString()} <span className="text-[11px] text-[#444870]">{paidPct}%</span>
                    </td>
                    <td className="px-3 py-2.5 tabular-nums text-[#e85d75]">{p.refund.toLocaleString()}</td>
                    <td className="px-3 py-2.5 tabular-nums text-[11px] text-[#e85d75]">{refundPct}%</td>
                    <td className="px-3 py-2.5 tabular-nums text-xs text-[#7e849e]">{sharePct}%</td>
                    <td className="px-3 py-2.5 w-24">
                      <div className="h-1.5 bg-[#2a2d45] rounded-full overflow-hidden">
                        <div className="h-full rounded-full transition-all" style={{ width: `${sharePct}%`, background: color }} />
                      </div>
                    </td>
                  </tr>
                )
          ...
          {productTotals.length > 0 && (
            <div className="px-4 py-2.5 border-t border-[#2a2d45] bg-[#1c1f2e] flex items-center justify-between text-xs text-[#444870]">
              ...
              <span>合计 {totalOrders.toLocaleString()} 单 · 退货 {totalRefund.toLocaleString()} 单</span>
            </div>
          )}
        </div>
      </div>
