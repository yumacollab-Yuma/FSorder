...
export async function GET(req: NextRequest) {
  ...
  if (!internalName) return NextResponse.json({ error: '缺少 product 参数' }, { status: 400 })

  // Use service_role key to bypass RLS
  const modaiq = createClient(
    process.env.MODAIQ_SUPABASE_URL!,
    process.env.MODAIQ_SERVICE_ROLE_KEY!
  )
  ...
  const sampledCreators: string[] = (data || [])
    .map((row: { data: unknown }) => {
      ...
      return (d as Record<string, unknown>)?.influencerId
    })
